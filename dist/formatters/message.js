...
function formatMessage(_a, state, messageDescriptor, values) {
    ...
    var id = messageDescriptor.id, defaultMessage = messageDescriptor.defaultMessage, message = messageDescriptor.message;
    // `id` is a required field of a Message Descriptor.
    invariant(id, '[React Intl] An `id` must be provided to format a message.');
    var messageWithId = utils_1.getFirstValidOption(message, messages && messages[id]);
    var hasValues = Object.keys(values).length > 0;
    // Avoid expensive message formatting for simple messages without values. In
    // development messages will always be formatted in case of missing values.
    if (!hasValues && process.env.NODE_ENV === 'production') {
        var val = utils_1.getFirstValidOption(messageWithId, defaultMessage, id);
