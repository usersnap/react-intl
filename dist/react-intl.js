...
(function (global, factory) {
    ...
    */
    ...
    var isStringOrNonEmptyArray = function isStringOrNonEmptyArray(val) {
      return typeof val === 'string' || Array.isArray(val) && val.length > 0;
    };
    var getFirstValidOption = function getFirstValidOption() {
      for (var _len = arguments.length, values = new Array(_len), _key = 0; _key < _len; _key++) {
        values[_key] = arguments[_key];
      }

      return values.find(isStringOrNonEmptyArray);
    };
    ...
    function formatMessage(_ref, state) {
      ...
      var id = messageDescriptor.id,
          ...
          message = messageDescriptor.message; // `id` is a required field of a Message Descriptor.

      invariant$2(id, '[React Intl] An `id` must be provided to format a message.');
      var messageWithId = getFirstValidOption(message, messages && messages[id]);
      var hasValues = Object.keys(values).length > 0; // Avoid expensive message formatting for simple messages without values. In
      // development messages will always be formatted in case of missing values.

      if (!hasValues && "development" === 'production') {
        var val = getFirstValidOption(messageWithId, defaultMessage, id);
